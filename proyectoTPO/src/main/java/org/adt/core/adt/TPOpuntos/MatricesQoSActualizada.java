...
public class MatricesQoSActualizada {
    ...
    public static void printQos2(QueueOfStacks a) {
        QueueOfStacks aux = new QueueOfStacks(3);
        while (!a.isEmpty()) {
            aux.add(a.getFirst());
            while (!a.getFirst().isEmpty()) {
                System.out.print(a.getFirst().getTop() + " ");
                a.getFirst().remove();
            }
            a.remove();
        }
    ...
    public static QueueOfStacks obtenerMatrizTraspuesta(QueueOfStacks qos) { //Complejidad computacional: O(n * m)
        IStack traspuesta= new Stack();
        QueueOfStacks matrizTraspuesta = new QueueOfStacks(3);
        while (!qos.isEmpty()){
            while(!qos.getFirst().isEmpty()){
                traspuesta.add(qos.getFirst().getTop());
                qos.getFirst().remove();
            }
            matrizTraspuesta.add(traspuesta);
            qos.remove();
        }
    ...
    public static void main(String[] args) {
        ...
        //printQos(obtenerMatrizTraspuesta(qos)); //TRASPUESTA
